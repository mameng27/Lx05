...
namespace DAL
{
    public class Activity
    {
        ...
        public Model.Activity GetRecomnend()//返回1个最新、审核通过、未过期并且推荐(recommend)的作品
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
        ...
        public int UpdateImg(Model.Activity active)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update activity set activityname=@activityName, activitypicture=@activityPicture,where activityid=@activityId";
                return cn.Execute(sql, active);
            }
        }
